...
# Evaluating fd_obj on the grid yields the continuous representations
...
# CRITERION: We define as outliers the 5% of observations with the lowest pseudo-density.
...
# Identify the indices and count of the outlier curves
...
plot_colors    <- rep(alpha("grey", 0.3), length(good_dates))
...
col_weekday <- alpha("steelblue", 0.35)
...
# Highlights afternoon sports during weekends and different nightlife/sleep patterns.
...
# SHARED PHASE: FPCA ON FUNCTIONAL QUANTILES (Monotone Rearrange --------
...
# ---> THE MAGIC OF FUNCTIONAL QUANTILES <---
# Sort the heart rates of each day (row) in descending order.
matrix_smooth <- t(apply(matrix_smooth, 1, sort, decreasing = TRUE))

# ---> LA SOLUZIONE: DOMAIN RESTRICTION <---
# Teniamo solo i primi 120 minuti (le 2 ore più intense della giornata).
# Tagliamo via le restanti 21 ore di riposo che "ingannano" l'algoritmo.
matrix_smooth <- matrix_smooth[, 10:130] 

# 2. Calculate global mean and sd (sulle curve tagliate!)
...
# 4. Discrete FPCA on the processed matrix (working_mat)
...
# 1. Calcolo metriche con range separati (Elbow parte da 1, Silhouette da 2)
...
# 2. Silhouette + Elbow Plots (Allineati visivamente da 1 a max_k)
...
# Ricostruzione delle curve usando SOLO le PC selezionate
reconstructed    <- scores %*% t(eigenvecs[, 1:n_pc])
approx_functions <- sweep(reconstructed, 2, global_mean, "+")

# Calcolo dei punteggi di conformità spaziale
...
# Teniamo solo le curve che rientrano nel limite conforme
mat_in <- approx_functions[in_band, , drop = FALSE]

# LA MAGIA È QUI: Definiamo esplicitamente la griglia temporale da 10 a 130
time_grid <- 10:130
...
cat("\n--- Plotting Original 24h Curves by Cluster ---\n")

# 1. Retrieve the ORIGINAL 24-hour smoothed curves (before quantile transformation)
matrix_smooth_original <- t(eval.fd(grid_min, fd_obj))

# 2. Create the dataframe and attach the cluster labels obtained from the transformed PCA
plot_df_orig <- as.data.frame(matrix_smooth_original)
colnames(plot_df_orig) <- grid_min
plot_df_orig$DayID   <- seq_len(nrow(plot_df_orig))
plot_df_orig$Cluster <- cluster_data_km$Label

# 3. Pivot to long format for ggplot2
plot_df_long_orig <- plot_df_orig %>%
  pivot_longer(cols = -c(DayID, Cluster), 
               names_to = "Minute", 
               values_to = "HR") %>%
  mutate(Minute = as.numeric(Minute))

# 4. Calculate the real 24h mean curve for each cluster
mean_curves_orig <- plot_df_long_orig %>%
  group_by(Cluster, Minute) %>%
  summarise(MeanHR = mean(HR, na.rm = TRUE), .groups = "drop")

# 5. Plot the original curves with facets
p_final_clusters_orig <- ggplot() +
  geom_line(data = plot_df_long_orig, 
            aes(x = Minute, y = HR, group = DayID, color = Cluster), 
            alpha = 0.3, linewidth = 0.5) +
  geom_line(data = mean_curves_orig, 
            aes(x = Minute, y = MeanHR), 
            color = "black", linewidth = 1.2, linetype = "dashed") +
  scale_x_continuous(breaks = seq(0, 1440, 180),
                     labels = seq(0, 1440, 180)) +
  facet_wrap(~ Cluster, ncol = 1) +
  scale_color_manual(values = cluster_palette) +
  labs(title = "Original 24h Heart Rate Profiles by Cluster",
       subtitle = "Clusters identified via Quantile FPCA, visualized on the original timeline",
